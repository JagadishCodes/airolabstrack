...
jQuery(document).ready(function($) {
    ...
    // Detect if the screen width is mobile or tablet
    function isMobile() {
        return window.innerWidth < 1024; // Adjust this value as needed for your breakpoint
    }
    ...
    if (isMobile()) {
        ...
        var tabbedMenus = $('#mega-menu-wrap-primary #mega-menu-primary > li.mega-menu-tabbed');
        tabbedMenus.each(function() {
            var $menu = $(this);
            var tabs = $menu.find('> ul.mega-sub-menu > li.mega-menu-item');
            if (tabs.length > 0) {
                ...
                var dynamicStyles = `
                    @media (max-width: 1023.98px) {
                        /* Hide nested submenus by default on mobile/tablet */
                        #${$menu.attr('id')} li.mega-menu-item > ul.mega-sub-menu {
                        ...
                        /* Show nested submenus only when their parent tab is toggled on mobile/tablet */
                        #${$menu.attr('id')} li.mega-menu-item.mega-toggle-on > ul.mega-sub-menu {
                ...
                // Force first tab to expand by default on mobile/tablet page load
                tabs.eq(0).addClass('mega-toggle-on open show').find('> a.mega-menu-link').attr('aria-expanded', 'true');
                // Ensure other tabs are collapsed on mobile/tablet page load
    ...
    // Toggle mega menu on tablet when hamburger is clicked (769px to 1023px) and manage body scrolling
    $(document).on('click', '#mega-menu-wrap-primary .mega-menu-toggle', function() {
        if (window.innerWidth > 768 && window.innerWidth < 1024) {
            $('#mega-menu-wrap-primary').toggleClass('mega-menu-open');
        }
        if (window.innerWidth < 1024) {
            setTimeout(function() {
                var isOpen = $('#mega-menu-wrap-primary').hasClass('mega-menu-open');
                if (isOpen) {
                    $('body, html').addClass('mega-menu-open-body');
                } else {
                    $('body, html').removeClass('mega-menu-open-body');
                }
            }, 50);
        }
    });
    ...
    $(window).on('resize', function() {
        if (window.innerWidth >= 1024) {
            $('body, html').removeClass('mega-menu-open-body');
            $('#mega-menu-wrap-primary').removeClass('mega-menu-open');
        }
    });
});
</script>
